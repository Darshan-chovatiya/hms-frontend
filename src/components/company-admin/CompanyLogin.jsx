...
const CompanyLogin = () => {
  ...
  try {
    ...
    if (role === 'hotel') {
      const res = await axios.post(`${BaseUrl}/company/login`, { email: emailId, password });
      result = res.data;
    } else {
      const res = await axios.post(`${BaseUrl}/driver/login`, { email: emailId, password });
      result = res.data;
    ...
    if (result.data) {
      ...
      localStorage.setItem('company', JSON.stringify(result.data.company || result.data.user));
      localStorage.setItem('companyRole', role); // 👈 Save role

      setErrorMsg('');
      navigate(role === 'hotel' ? '/hotel' : '/driver');
    } else {
  ...
  <select className="form-select" value={role} onChange={(e) => setRole(e.target.value)}>
    ...
    <option value="driver">Driver</option>
          ...
          <div className="text-center mt-4">
            <p className="text-muted small mb-0">Secure hotel authentication portal</p>
            {/* {role === "driver" && ( */}
    <p className="mt-2">
      Don't have an account?{" "}
      <a href="/driver-signup" className="text-primary fw-bold">Sign up as Driver</a>
    </p>
  {/* )} */}
          </div>
        </div>
